...
def clean_text(text):
    ...
    # At this stage we convert the lemmatized tokens into a string again
    # the difference between this cleaning function and the one we use 
    # when we only lemmatize is this one returns a string while the former one 
    # returns a list of words
    lemmatized_tokens_str = " ".join([wn.lemmatize(token) for token in tokens_NoStop])
...
f = open("Lemmatized Ngram GradientBoost.txt", "w")
